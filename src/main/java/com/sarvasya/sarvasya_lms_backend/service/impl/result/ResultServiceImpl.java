package com.sarvasya.sarvasya_lms_backend.service.impl.result;

import com.sarvasya.sarvasya_lms_backend.model.exam.Exam;
import com.sarvasya.sarvasya_lms_backend.model.result.Result;
import com.sarvasya.sarvasya_lms_backend.model.result.ResultItem;
...
import com.sarvasya.sarvasya_lms_backend.repository.result.ResultRepository;
    ...
    public byte[] generatePdf(UUID resultId, UUID userId) throws Exception {
        ...
        if (result.getItems() != null) {
            for (ResultItem item : result.getItems()) {
                ...
                itemsHtml.append("<tr>")
                        ...
                        .append(item.getMarksObtained()).append("</td>")
                        .append("<td style='border:1px solid #ccc; padding:8px; text-align:center;'>")
                        .append(item.getTotalMarks()).append("</td>")
                        .append("</tr>");
            }
        }
        ...
        String html = "<!DOCTYPE html><html><head><style>" +
                ...
                (result.getGrade() != null && !result.getGrade().isEmpty()
                        ? "<div class='grade'>Overall Grade: " + result.getGrade() + "</div>"
                        : "")
                +
